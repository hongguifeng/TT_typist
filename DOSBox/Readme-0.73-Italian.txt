...
R: Per montare il tuo CD-ROM in DOSBox devi specificare qualche opzione addizionale
   ...
   Nei comandi: - d   lettera di drive che avrai nel DOSBox
                ...
                - 0   Il numero del drive CD-ROM, ottenuto tramite mount -cd
                      (nota che questo valore è necessario solo quando si usa SDL per i
...
dosbox -eraseconf
  ...
  -machine tipomacchina
        ...
        vgaonly, nota che ciò disattiva le funzionalità svga e potrebbe
        essere (considerevolmente) più lento a causa della maggiore precisione
  ...
  -editconf programma
        ...
        Puoi specificare questo comando più volte. In questo caso, avvierà
...
MOUNT -u "Lettera Drive Emulato"
  ...
  -label etichettadrive
        ...
        quando specifichi -noioctl):
          Per Windows: l'etichetta è estratta dal "Drive Reale".
  ...
  -ioctl_mci (MCI usato per i CD audio)
        ...
        Le varie scelte variano solo nel modo in cui il CD audio è gestito,
        preferibilmente viene usato -ioctl_dio (il più leggero), ma potrebbe non
        funzionare su tutti i sistemo, quindi si può anche provare con -ioctl_dx
        (o -ioctl_mci).
  ...
  -usecd numero
        ...
        nessuno. "numero" può essere ricavato digitando "MOUNT -cd".
...
VER set versione_maggiore [versione_minore]
  ...
  per esempio: "VER set 6 22" per far sì che DOSBox riporti DOS 6.22
  come versione DOS.
...
CONFIG -get "sezione proprietà"
  ...
  -securemode
       Imposta DOSBox su una modalità più sicura. In questa modalità i
       comandi interni MOUNT, IMGMOUNT e BOOT non funzionano. Non è inoltre
       possibile creare un nuovo file di configurazione o di linguaggio.
       (Attenzione: puoi annullare questa modalità solo riavviando DOSBox.)
...
MIXER
  ...
  canale
      Può essere uno dei seguenti: MASTER, DISNEY, SPKR, GUS, SB, FM [, CDAUDIO].
      CDAUDIO è disponibile solo se è disponibile una interfaccia CD-ROM con
  ...
  /LISTMIDI
      ...
      file di configurazione, dove 'id' è il numero del dispositivo
      così come elencato da LISTMIDI.
...
IMGMOUNT
  ...
  fileimmagine
      ...
      CUE/BIN è il formato preferito per le immagini CD-ROM perché possono
  ...
  fileimmag1, .. ,fileimmagN
      ...
      più file immagine solo per le immagini CD-ROM. I CD possono poi essere
      cambiati con CTRL-F4 in qualsiasi momento. Ciò è richiesto dai giochi
      che usano più CD-ROM e richiedono che il CD venga sostituito nel bel
      mezzo della partita.
...
Questa capacità varia da computer a computer.
...
D3. Provando in DOSBox, noti che la pressione di X fa apparire ZX.
     R. La X sulla tua tastiera è ancora mappata alla X! Clicca sulla X della
...
nel file di configurazione (la voce mapperfile= ). All'avvio, DOSBox caricherà
...
sotto windows, il layout è scelto in accordo a quello del SO.
 ...
 * inhsocket:1   - Usa un socket passato a DOSBox da linea di comando. Imposta
                   transparent automaticamente. (Eredità dei Socket: Viene usato
...
DOSBox caricherà i file di configurazione che sono specificati da -conf.
Se non ve n'è nessuno, proverà a caricare "dosbox.conf" dalla directory
locale. Se non c'è, DOSBox caricherà il file di configurazione utente.
Questo file, se inesistente, verrà creato. Il file può essere trovato
